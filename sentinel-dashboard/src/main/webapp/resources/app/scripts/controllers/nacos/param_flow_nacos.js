 ...
 * @author Eric Zhao
 */
angular.module('sentinelDashboardApp').controller('ParamFlowNacosController', ['$scope', '$stateParams', 'ParamFlowNacosService', 'ngDialog',
  'MachineService',
  function ($scope, $stateParams, ParamFlowNacosService, ngDialog,
    MachineService) {
    ...
    function getMachineRules() {
      ...
      ParamFlowNacosService.queryMachineRules($scope.app, mac[0], mac[1])
    ...
    $scope.saveRule = function () {
      if (!ParamFlowNacosService.checkRuleValid($scope.currentRule.rule)) {
        return;
      }
    ...
    function addNewRuleAndPush(rule) {
      ParamFlowNacosService.addNewRule(rule).success((data) => {
        if (data.success) {
          getMachineRules();
    ...
    function saveRuleAndPush(rule, edit) {
      ParamFlowNacosService.saveRule(rule).success(function (data) {
        if (data.success) {
          // alert("修改规则成功");
          getMachineRules();
          if (edit) {
    ...
    function deleteRuleAndPush(entity) {
      ...
      ParamFlowNacosService.deleteRule(entity).success((data) => {
        if (data.code == 0) {
          getMachineRules();
          confirmDialog.close();
        } else {
