...
app.controller('SystemNacosCtl', ['$scope', '$stateParams', 'SystemNacosService', 'ngDialog', 'MachineService',
  function ($scope, $stateParams, SystemNacosService,
    ...
    function getMachineRules() {
      ...
      SystemNacosService.queryMachineRules($scope.app, mac[0], mac[1]).success(
        function (data) {
    ...
    function deleteRule(rule) {
      SystemNacosService.deleteRule(rule).success(function (data) {
        if (data.code === 0) {
          getMachineRules();
          confirmDialog.close();
    ...
    function addNewRule(rule) {
      ...
      SystemNacosService.newRule(rule).success(function (data) {
        if (data.code === 0) {
          getMachineRules();
    ...
    function saveRule(rule, edit) {
      SystemNacosService.saveRule(rule).success(function (data) {
        if (data.code === 0) {
          getMachineRules();
          if (edit) {
            systemRuleDialog.close();
