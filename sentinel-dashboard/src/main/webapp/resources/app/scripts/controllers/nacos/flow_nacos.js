...
app.controller('FlowNacosController', ['$scope', '$stateParams', 'FlowNacosService', 'ngDialog',
  'MachineService',
  function ($scope, $stateParams, FlowNacosService, ngDialog,
    MachineService) {
    ...
    function getMachineRules() {
      ...
      FlowNacosService.queryMachineRules($scope.app, mac[0], mac[1]).success(
        function (data) {
    ...
    $scope.saveRule = function () {
      if (!FlowNacosService.checkRuleValid($scope.currentRule)) {
        return;
      }
    ...
    function deleteRule(rule) {
      FlowNacosService.deleteRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
          confirmDialog.close();
        } else {
    ...
    function addNewRule(rule) {
      FlowNacosService.newRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
    ...
    function saveRule(rule, edit) {
      FlowNacosService.saveRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
          if (edit) {
            flowRuleDialog.close();
